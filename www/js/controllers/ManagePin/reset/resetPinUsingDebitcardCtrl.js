angular.module('ctrl.resetPinUsingDebitcardCtrl', [])
	.controller('resetPinUsingDebitcardStep1Ctrl', function ($scope, $state, popupService, mainSession, kkconst, subscriptionService, managePinService) {
		console.log('resetPinUsingDebitcardStep1Ctrl');
	...
	.controller('resetPinUsingDebitcardStep2Ctrl', function ($scope, $state, popupService, mainSession, kkconst, managePinService, deviceService, subscriptionService, webStorage) {
		console.log('resetPinUsingDebitcardStep2Ctrl');
		...
		$scope.event = {
			...
			isShowNext: false,
			goBackPage: function(){
				$state.go(kkconst.ROUNTING.RESET_PIN_USING_DEBITCARD_STEP1.STATE);
			},
			goNextPage: function(){
				// do something
			}
